...
async function performRequest(endpoint, body, method) {
  ...
  if (method === 'GET') {
    ...
    try {
      ...
      toast.error(Trans.currentLocale === 'uk' ? 'Помилка оновлення' : 'Update error')
      response = 'error'
    }
  } else {
...
export const handleRequestWithQueue = async (endpoint, body = null, method = 'GET') => {
  if (canMakeRequest()) {
    isFetching = true
...
const handleError = (error) => {
  const errorMessage = error.response?.data?.detail || t('fetchError')
